/**********************************************************************************
*\file         RestartLevelWindow_Button_Game.h
*\brief        RestartLevelWindow_Button_Game.h
*
...
struct restartlevel_window_button_game_script : paperback::script::button_interface // Inherited Type (1)
{
    static constexpr auto typedef_v = paperback::script::type::button
    {
        .m_pName = "Restart Level Window Button Game Script"
    };

    // You have to define this constructor - For internal registration
    restartlevel_window_button_game_script(paperback::coordinator::instance& Instance) noexcept :
    ...
    void OnClick() noexcept override
    {
        ...
        PPB.GetSystem<ui_system>().ToggleLayerObjects(static_cast<int>(UI_LAYER::RESTART), true);
    }
};